          ...
          <div className="col-md-12">
            <div className="card">
              <div className="card-header row">
                <div className="col-6">
                  <div className="card-title">Manajemen Data Users</div>
                  <div className="card-category">Tabel Users</div>
                </div>
                <div className="col-6 text-right">
                  <AllowAction module="users" action="create">
                      ...
                      {data.data.length > 0 ? (
                        data.data.map((items, index) => (
                          <tr key={index}>
                            ...
                            <td>{items.email}</td>
                            <td>
                              {(items.role_name != null && (items.role_name.split(";;").map((items_role,index_role) => (
                                <span className="badge badge-success mr-1" key={index_role}>{items_role}</span>
                              ))))}
                            </td>
                            <td>
                              <div className="form-button-action">
                                <AllowAction module="users" action="read">
                                  <Detail>
                                    {route("users.show", items.id_user)}
                                  </Detail>
                                </AllowAction>

                                <AllowAction module="users" action="update">
                                  <Edit>
                                    {route("users.edit", items.id_user)}
                                  </Edit>
                                </AllowAction>

                                <AllowAction module="users" action="delete">
                                  <Delete
                                    url={route(
                                      "users.destroy",
                                      items.id_user
