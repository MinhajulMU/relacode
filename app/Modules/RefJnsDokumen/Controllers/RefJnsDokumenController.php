...
class RefJnsDokumenController extends Controller
{
    protected $title = "Jenis Dokumen";
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('RefJnsDokumen::Index', $data);
    }
    public function create()
    {
        $data['routes']['backUrl'] =  url()->previous() == URL::to('/') || url()->previous() == URL::current() ? URL::route('ref-jns-dokumen.index') : url()->previous();

        return Inertia::render('RefJnsDokumen::Create', $data);
    }
    public function store(Request $request)
    {
        ...
        $refJnsDokumen = $this->model->create($formData);
        $log  = Log::aktivitas('Menambah ' . $this->title . ' ID = ' . $refJnsDokumen->id_jns_dokumen);
        return redirect()
    ...
    public function show($id)
    {
        ...
        $data['routes']['backUrl'] =  url()->previous() == URL::to('/') || url()->previous() == URL::current() ? URL::route('ref-jns-dokumen.index') : url()->previous();
        return Inertia::render('RefJnsDokumen::Show', $data);
    }

    public function edit($id)
    {
        $data['refJnsDokumen'] = $this->model::findOrFail($id);

        $data['routes']['backUrl'] =  url()->previous() == URL::to('/') || url()->previous() == URL::current() ? URL::route('ref-jns-dokumen.index') : url()->previous();
        return Inertia::render('RefJnsDokumen::Edit', $data);
    }
